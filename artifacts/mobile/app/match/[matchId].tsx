...
function MarketCard({ market, onPredict }: { market: any; onPredict: (m: any) => void }) {
  ...
  const STATUS_LABELS: Record<string, string> = {
    open: t('match_open'), paused: t('match_paused'), closed: t('match_closed'),
    settled: t('match_settled'), refunded: t('match_refunded'),
  };
  ...
  return (
    <View style={s.card}>
      <View style={s.cardHeader}>
        ...
        <View style={[s.statusBadge, { backgroundColor: (STATUS_COLORS[market.status] || colors.muted) + '22' }]}>
          <Text style={[s.statusText, { color: STATUS_COLORS[market.status] || colors.mutedForeground }]}>
      ...
      {isOpen && (
        <View style={s.buttons}>
          ...
          <TouchableOpacity style={[s.btn, s.noBtn]} onPress={() => onPredict({ ...market, preChoice: 'NO' })} activeOpacity={0.8}>
            <Text style={s.btnLabel}>{t('match_no')}</Text>
...
export default function MatchDetailScreen() {
  ...
  const CATEGORY_LABELS: Record<string, string> = {
    toss: t('match_cat_toss'), innings: t('match_cat_innings'), over: t('match_cat_over'),
    batsman: t('match_cat_batsman'), bowler: t('match_cat_bowler'), match_winner: t('match_cat_winner'),
  };
    ...
    <View style={s.root}>
      ...
      {predicted && (
        <View style={s.successToast}>
          ...
          <Text style={s.successText}>{t('match_success')}</Text>
        </View>
      )}
      ...
      <ScrollView contentContainerStyle={s.list}>
        {marketsLoading ? (
          ...
          <View style={s.empty}>
            ...
            <Text style={s.emptyText}>{t('match_no_markets')}</Text>
          </View>
        ) : (
          Object.entries(grouped).map(([cat, mktList]) => (
            <View key={cat}>
              <Text style={s.catLabel}>{CATEGORY_LABELS[cat] || cat}</Text>
              {mktList.map((m) => <MarketCard key={m.id} market={m} onPredict={setSelectedMarket} />)}
            </View>
          ))
        )}
      </ScrollView>
      ...
      <Modal visible={!!selectedMarket} transparent animationType="slide">
        <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => { setSelectedMarket(null); setAmount(''); }}>
          <TouchableOpacity activeOpacity={1} style={s.sheet} onPress={() => {}}>
            ...
            <Text style={s.sheetTitle}>{t('match_predict_title')}</Text>
            ...
            <View style={[s.choiceBadge, {
              ...
              <Text style={[s.choiceText, { color: selectedMarket?.preChoice === 'YES' ? '#22C55E' : '#EF4444' }]}>
                {selectedMarket?.preChoice === 'YES' ? `${t('match_yes')} ✓` : `${t('match_no')} ✗`}
                {'  '}×{selectedMarket
                  ? (selectedMarket.preChoice === 'YES'
                      ? Number(selectedMarket.yesPrice).toFixed(2)
                      : Number(selectedMarket.noPrice).toFixed(2))
                  : '0'}
              </Text>
            </View>

            <Text style={s.amountLabel}>{t('match_enter_amount')}</Text>
            <TextInput
              style={s.amountInput}
              placeholder="₹100"
            ...
            <View style={s.quickAmounts}>
              {[100, 200, 500, 1000].map((a) => (
                <TouchableOpacity key={a}
                  style={[s.quickBtn, amt === a && { borderColor: colors.primary, backgroundColor: colors.primary + '20' }]}
            ...
            <Text style={s.balanceText}>{t('match_wallet_balance', user?.walletBalance?.toFixed(0) ?? '0')}</Text>
            <TouchableOpacity
            ...
            >
              {placePrediction.isPending
                ...
                : <Text style={s.confirmText}>{t('match_confirm')}</Text>
              }
            </TouchableOpacity>
          </TouchableOpacity>
        </TouchableOpacity>
...
const styles = (colors: ReturnType<typeof useColors>, insets: any) => StyleSheet.create({
  ...
  breakdown: { backgroundColor: colors.muted, borderRadius: 12, padding: 14, marginBottom: 14, borderWidth: 1, borderColor: colors.border, gap: 8 },
