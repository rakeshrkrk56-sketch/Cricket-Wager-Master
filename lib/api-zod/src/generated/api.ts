...
export const GetWalletResponse = zod.object({
  "userId": zod.string(),
  "balance": zod.number(),
  "depositTotal": zod.number().describe('Total approved deposit funds credited to the wallet. These funds are playable but not withdrawable.'),
  "depositBalance": zod.number().describe('Remaining deposited funds available for play. These funds are not withdrawable.'),
  "withdrawTotal": zod.number().describe('Total withdrawal amount already approved and processed.'),
  "winTotal": zod.number().describe('Total winnings credited from settled games and predictions.'),
  "bonusTotal": zod.number().describe('Total bonus funds credited to the wallet. Bonus funds are playable but not withdrawable.'),
  "bonusBalance": zod.number().describe('Remaining bonus funds available for play. These funds are not withdrawable.'),
  "pendingWithdrawalTotal": zod.number().describe('Withdrawal requests awaiting admin approval.'),
  "withdrawableWinnings": zod.number().describe('Current winnings available to withdraw after approved and pending withdrawals, capped by the wallet balance.'),
...
export const CreateSupportTicketResponse = zod.object({
  ...
  "hasScreenshot": zod.boolean(),
  "screenshotBase64": zod.string().nullish().describe('Attached support image, included on ticket detail responses only'),
